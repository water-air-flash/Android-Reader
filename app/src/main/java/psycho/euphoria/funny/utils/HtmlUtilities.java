...
public class HtmlUtilities {
    ...
    private static final Pattern sPatternTag = Pattern.compile("<.*?>", Pattern.DOTALL);
    private static final Pattern sPatternEmpty = Pattern.compile(" {2,}");
    private static final Pattern sPatternTitle = Pattern.compile("<title>(.*?)</title>", Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
    private static final Pattern sPatternBody = Pattern.compile("<body>", Pattern.CASE_INSENSITIVE);
    private static final Pattern sPatternNewLine = Pattern.compile("(?m)^[ \t]*\r?\n");

    public static Pair<String, String> html2txt(String str) {
        String r = "";
        String t = "";
        r = str.replaceAll("<\\s+", "<");
        r = r.replaceAll("<\\s+/", "</");
        r = r.replaceAll("\\s+>", ">");
        r = r.replaceAll("[\r\n]", " ");
        final Matcher matcher = sPatternTitle.matcher(r);
        if (matcher.find()) {
        ...
        final Matcher matcherPattern = sPattern.matcher(r);
        if (matcherPattern.find()) {
            r = matcherPattern.replaceAll("\n");

        }
        final Matcher matcherTag = sPatternTag.matcher(r);
        if (matcherTag.find()) {
            r = matcherTag.replaceAll("");

        }
        final Matcher matcherEmpty = sPatternEmpty.matcher(str);
        if (matcherEmpty.find()) {
            r = sPatternEmpty.matcher(r).replaceAll(" ");

        }
        final Matcher matcherNewLine = sPatternNewLine.matcher(str);
        if (matcherNewLine.find()) {
            r = sPatternNewLine.matcher(r).replaceAll("\n").replaceAll("\n+", "\n\n");

        }
        for (String[] s : BASIC_ESCAPE) {
            r = r.replaceAll(s[1], s[0]);
        }
        for (String[] s : ISO8859_1_ESCAPE) {
            r = r.replaceAll(s[1], s[0]);
        }
        if (t == null)
            t = "";
        return Pair.create(t, r.trim());
    }
}
